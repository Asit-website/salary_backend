      ...
      try {
        ...
        if (lpResult && lpResult.isLate) {
          ...
          try {
            ...
            try {
              const shiftTpl = await shiftService.getEffectiveShiftTemplate(staff.id, dateKey);
            ...
            const newMessage = `${staffName} has checked in late via Kiosk today by ${notificationMins} minutes.`;
